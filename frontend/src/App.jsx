            ...
            <div className="flex gap-6 mb-6 max-[768px]:flex-col max-[768px]:gap-3 max-[768px]:mb-[14px]">
              ...
              <div>
                {(() => {
                  ...
                  const renderEmptyRow = (isCombined) => {
                    const gridCols = isCombined 
                      ? 'md:grid-cols-[minmax(220px,2.05fr)_minmax(145px,1.12fr)_minmax(118px,0.9fr)_minmax(150px,1.02fr)_minmax(130px,0.9fr)_220px]' 
                      : 'md:grid-cols-[minmax(220px,2.2fr)_minmax(118px,0.95fr)_minmax(150px,1.05fr)_minmax(130px,0.95fr)_220px]';
                    return (
                      <div className={`border-b border-[#1E293B] bg-[#081329] px-5 py-5 text-[0.95rem] font-normal text-[#64748B] max-[768px]:block md:grid md:items-center md:gap-4 ${gridCols}`}>
                        <div className="flex items-center justify-center max-[768px]:py-4">N/A</div>
                        {isCombined && <div className="hidden items-center justify-center md:flex">N/A</div>}
                        <div className="hidden items-center justify-center md:flex">N/A</div>
                        <div className="hidden items-center justify-center md:flex">N/A</div>
                        <div className="hidden items-center justify-center md:flex">N/A</div>
                        <div className="hidden items-center justify-center md:flex">N/A</div>
                      </div>
                  ...
                  if (viewMode === 'combined') {
                    ...
                    return sorted.length ? sorted.map(r => renderResultCard(r, true)) : renderEmptyRow(true);
                  } else {
                    const bankItems = sortResults(filtered.filter(r => r.productType === 'Bank CDs'));
                    const brokeredItems = sortResults(filtered.filter(r => r.productType === 'Brokerage CDs'));
                    const treasuryItems = sortResults(filtered.filter(r => r.productType === 'Treasuries'));
                    ...
                    return (
                      <>
                        ...
                        {bankItems.length ? bankItems.map(r => renderResultCard(r, false)) : renderEmptyRow(false)}
                        ...
                        {brokeredItems.length ? brokeredItems.map(r => renderResultCard(r, false)) : renderEmptyRow(false)}
                        ...
                        {treasuryItems.length ? treasuryItems.map(r => renderResultCard(r, false)) : renderEmptyRow(false)}
                      </>
                    );
                  }
                })()}
